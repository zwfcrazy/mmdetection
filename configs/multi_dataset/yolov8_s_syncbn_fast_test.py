...
data_root = '/mnt/user/zhuwenfei/human_pose/data/'
...
weak_pipeline = [
    ...
    dict(type='DummyScale'),
    dict(
        type='PackDetInputs',
        meta_keys=('img_id', 'img_path', 'ori_shape', 'img_shape', 'flip',
                   'flip_direction', 'scale_factor', 'homography_matrix')),
]
...
train_pipeline = [
    dict(
        type='SharedMultiBranch',
        shared_pipeline=shared_pipeline,
        branch_field=branch_field,
        teacher=weak_pipeline,
        student=strong_pipeline,
    )
]
...
train_dataloader = dict(
    ...
    collate_fn=dict(
        type='semi_collate',
        teacher_collate_fn=dict(type='pseudo_collate'),
        student_collate_fn=dict(type='semi_yolov5_collate')
        ),
    dataset=dict(
...
detector = dict(
    ...
    data_preprocessor=dict(
        type='SemiYOLOv5DetDataPreprocessor',
        mean=[0., 0., 0.],
        std=[255., 255., 255.],
        bgr_to_rgb=True),
    backbone=dict(
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint='https://download.openmmlab.com/mmyolo/v0/yolov8/yolov8_s_syncbn_fast_8xb16-500e_coco/yolov8_s_syncbn_fast_8xb16-500e_coco_20230117_180101-5aa5f0f1.pth'  # noqa
    ),
    train_cfg=dict(
        assigner=dict(
...
model = dict(
    type='MultiDatasetDetector',
    detector=detector,
    data_preprocessor=dict(
        type='SemiDataPreprocessor',
...
custom_hooks = [dict(type='MeanTeacherHook', momentum=0.001, skip_buffer=False)]  # skip_buffer must be false
